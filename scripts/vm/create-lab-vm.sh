...
DEFAULT_ISO="${REPO_ROOT}/output/xedra-0.4-amd64.iso"
ISO_PATH="${1:-${DEFAULT_ISO}}"

print_header() {
    echo -e "${COLOR_BOLD}${COLOR_CYAN}======================================================${COLOR_RESET}"
    echo -e "${COLOR_BOLD}${COLOR_CYAN}  Xedra Linux - Launch 'xedra-lab' Test VM (v0.4)     ${COLOR_RESET}"
    echo -e "${COLOR_BOLD}${COLOR_CYAN}======================================================${COLOR_RESET}"
...
verify_environment() {
    ...
    if [[ ! -f "${ISO_PATH}" ]]; then
        ...
        echo "  scp builder@192.168.122.180:~/XedraLinux/output/xedra-0.4-amd64.iso ${REPO_ROOT}/output/"
        echo "  (or for minimal: scp builder@192.168.122.180:~/XedraLinux/output/xedra-0.4-minimal-amd64.iso ${REPO_ROOT}/output/)"
        exit 1
    fi
