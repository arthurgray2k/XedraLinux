...
This document provides visual architectural diagrams, dependency hierarchies, and end-to-end execution flowcharts for Xedra Linux 0.4.1.
...
graph TD
    ...
    subgraph Userland_Tools ["3. Languages, Toolchains & Editors"]
        T1["Python 3 (python3, pip, venv)"]
        T2["Golang (golang-go compiler)"]
        T3["Editors (micro, nano, vim-tiny)"]
    end

    subgraph Disk_Installer ["4. Native System Installer Subsystem"]
        ...
        D6["grub-efi-amd64 (UEFI Bootloader)"]
        D7["grub-pc-bin (BIOS MBR Modules)"]
        D8["grub2-common (grub-install & update-grub)"]
    end

    subgraph GUI_Desktop ["5. Display & Desktop Subsystem (GUI Only)"]
...
flowchart TD
    ...
    LBConfig --> GenPackages["Generate package-lists/xedra.list.chroot (Kernel, Python, Go, Micro, Installer, Fluxbox)"]
    GenPackages --> GenHooks["Install 0100-sysvinit-transition.hook.chroot"]
    ...
    subgraph LiveBuild_Stages ["Internal live-build Engine"]
        ...
        ChrootInstall --> ChrootHook["3. lb hooks: Execute SysVinit transition & set passwords"]
    ...
    BinaryISO --> PackageArtifacts["Move to output/xedra-0.4.1-amd64.iso & generate .sha256"]
...
flowchart TD
    ...
    DialogSelect --> ConfirmDialog["dialog --yesno: Confirm Permanent Disk Overwrite"]
    ...
    GenFstab --> BindChroot["Bind Mount: /dev, /proc, /sys into /mnt/xedra-target"]
    BindChroot --> CheckFirmware{"Firmware Mode?"}

    CheckFirmware -->|UEFI Mode| GrubUEFI["chroot: grub-install --target=x86_64-efi --efi-directory=/boot/efi"]
    ...
    CleanUnmount --> SuccessMsg(["dialog: Installation Successful -> Prompt for Reboot"])
```
...
sequenceDiagram
    ...
    participant Services as rcS & rc 2 Runlevels
    participant Session as TTY1 / Fluxbox Desktop

    Hardware->>ESP: Locate \\EFI\\XedraLinux\\grubx64.efi
    ...
    Init->>Services: Execute /etc/init.d/rc 2 (dbus, elogind, networking)
    Init->>Session: /sbin/getty auto-login user 'xedra' on tty1
